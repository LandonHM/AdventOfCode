...
fn main() {
    ...
    for line in read_to_string("input.txt").unwrap().lines() {
        ...
        let id = &split.next().unwrap()[5..].parse::<usize>().unwrap();
        ...
        if excedes_max(&max_allowed, &max) {
            println!("{id}");
            sum += id;
        }
    }
...
fn excedes_max(max: &[usize], arr: &[usize]) -> bool {
    for (i,j) in max.iter().zip(arr.iter()) {
        if j > i {
            return false;
        }
    }
